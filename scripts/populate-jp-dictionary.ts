...
async function processFile(filePath: string, seenWords: Set<string>): Promise<JpDictEntry[]> {
...
async function main() {
  ...
  try {
    ...
    const jsonFiles = files.filter(file => file.startsWith('term_bank_') && file.endsWith('.json'))
    ...
    for (const file of jsonFiles) {
      ...
      const entries = await processFile(path.join(directoryPath, file), seenWords)
